...
export async function chunkData(data: Uint8Array): Promise<Chunk[]> {
  ...
  while (rest.byteLength >= MAX_CHUNK_SIZE) {
    ...
    const dataHash = await Bigfile.crypto.hash(chunk);
  ...
  chunks.push({
    dataHash: await Bigfile.crypto.hash(rest),
    minByteRange: cursor,
...
/**
 * Builds an bigfile merkle tree and gets the root hash for the given input.
 */
...
async function hash(data: Uint8Array | Uint8Array[]) {
  if (Array.isArray(data)) {
    data = Bigfile.utils.concatBuffers(data);
  }

  return new Uint8Array(await Bigfile.crypto.hash(data));
}
...
/**
 * Inspect an bigfile chunk proof.
