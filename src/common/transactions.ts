...
declare const bigfileWallet: Window["bigfileWallet"];
...
export default class Transactions {
  ...
  public async sign(
    ...
    const externalWallet = typeof bigfileWallet === "object";
    ...
    if (!validJwk && !externalWallet) {
      ...
      try {
        const existingPermissions = await bigfileWallet.getPermissions();

        if (!existingPermissions.includes("SIGN_TRANSACTION"))
          await bigfileWallet.connect(["SIGN_TRANSACTION"]);
      } catch {
        // Permission is already granted
      }

      const signedTransaction = await bigfileWallet.sign(transaction, options);
   ...
   *
   * ```
   * const uploader = bigfile.transactions.getUploader(transaction);
   * while (!uploader.isComplete) {
   *   await uploader.uploadChunk();
