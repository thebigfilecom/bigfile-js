...
//@ts-ignore
const bigfile: Bigfile = self.Bigfile.init({
...
describe("Initialization", function () {
  it("should have components", function () {
    expect(bigfile.api).to.an("object");

    expect(bigfile.transactions).to.an("object");

    expect(bigfile.wallets).to.an("object");

    expect(bigfile.network).to.an("object");

    expect(bigfile.crypto).to.an("object");

    expect(bigfile.silo).to.an("object");
  });
});
...
describe("Transactions", function () {
  ...
  it("should create and sign transactions using external implicit wallet", async function () {
    ...
    const transaction = await bigfile.createTransaction({ data: "test" });
    await bigfile.transactions.sign(transaction);
  ...
  it("should get transaction info", async function () {
    ...
    const transaction = await bigfile.transactions.get(liveDataTxid);
    ...
    const verify = await bigfile.transactions.verify(transaction);
    ...
    const verifyResult = await (() => {
      return new Promise((resolve) => {
        bigfile.transactions.verify(transaction).catch((error: any) => {
          resolve(error);
        });
      });
    })();
...
describe("Silo Web", function () {
  it("should read Silo transaction", async function () {
    ...
    // data = 'something'
    // uri = 'secret.1'
    const transaction = bigfile.transactions.fromRaw({
    ...
    const decrypted = await bigfile.silo.readTransactionData(
      transaction,
      "thing.1"
    );
  ...
  it("should pass a Silo transaction roundtrip", async function () {
    ...
    const transaction = await bigfile.createSiloTransaction(
      { data: "test data" },
      wallet,
      "my-silo-ref.1"
    );
    ...
    let decrypted = await bigfile.silo.readTransactionData(
      transaction,
      "my-silo-ref.1"
    );
...
describe("GraphQL", function () {
  ...
  it("should return a list of results", async function () {
    const txs = (
      await bigfile.api.post("/graphql", {
        query: `
      {
        transactions(
          tags: [
